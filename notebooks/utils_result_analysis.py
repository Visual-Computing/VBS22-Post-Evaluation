...
#                 'rank_shot_margin_0']
#############BOX PLOT ########
def boxplot_best_rank (df, output_dir,max_records=10000,rank_column_name="rank_shot_margin_0",
                       figsize = [7, 6],  show_boxplot = True, swarmplot = True, fill_missing=False, show_missing = False,  ylogscale=True):
        where_missing_data_is = max_records * 2

        view = df[df[rank_column_name] != -1].groupby(['teamFamily', 'user']).agg('count')[rank_column_name]
        print(view)
        #missing datapoints are set in some random lines after max_records + k
        if fill_missing:
                df.sort_values(by=[rank_column_name])
                df.loc[df[rank_column_name] == -1, rank_column_name] = np.random.uniform(
                        where_missing_data_is * 10 ** (-0.05),
                        where_missing_data_is * 10 ** (0.05),
                        len(df.loc[df[rank_column_name] == -1]))

        # select only r_s
        df = df[[rank_column_name, "teamFamily", "user", "task"]]
        ...
        if show_boxplot:
                sns.boxplot(x="teamFamily", hue="user", y=rank_column_name, data=df[df[rank_column_name] != -1], whis=[0, 100], width=.6,
                            palette="vlag")
        if show_missing:
                df.sort_values(by=[rank_column_name])
                df.loc[df[rank_column_name] == -1, rank_column_name] = np.random.uniform(
                ...
                ax.axhline(y=where_missing_data_is, linewidth=25, color='r', alpha=0.2)
        else:
                ax.set_yticks([10 ** x for x in r])
                ax.set_yticklabels(['1' if x == 0 else '10' if x == 1 else f'10$^{x}$' for x in r])

        # discard NaN values
        #df = df[df[rank_column_name] != -1]
        # Add in points to show each observation

        if swarmplot:
                sns.swarmplot(x="teamFamily", hue="user", y=rank_column_name, data=df,
                              size=4, linewidth=.3, dodge=True, alpha=0.4 if show_boxplot else 1.0)
        else:
                sns.stripplot(x="teamFamily", hue="user", y=rank_column_name, data=df,
        ...
        #       labels[0] = "best"
        #       labels[1] = "other"
        ax.legend(handles[:4], labels[:4], title="user", loc="upper right")  # , ncol=2,)

        # Tweak the visual presentation
        ax.yaxis.grid(True)
        ylabel={"rank_shot_margin_0":"best frame/shot rank", "rank_video":"best video rank", "score": "DRES Score"}
        ax.set(xlabel="Teams")
        ax.set(ylabel=ylabel[rank_column_name])
        # sns.despine(trim=True, left=True)
        plt.savefig(f'{output_dir}/kis_{rank_column_name}_boxplot_shotrank{max_records}.pdf', format='pdf', bbox_inches="tight")